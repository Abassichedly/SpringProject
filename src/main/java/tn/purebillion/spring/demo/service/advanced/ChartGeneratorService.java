...
public class ChartGeneratorService {
    ...
    public ChartDataDTO generateMemberEvolutionChart(Long clubId) {
        ...
        List<Membre> membres = club.getMembres();

        // Regrouper par mois d'adhésion
        Map<String, Long> monthlyData = membres.stream()
                .filter(m -> m.getDateAdhesion() != null)
                .collect(Collectors.groupingBy(
                        m -> m.getDateAdhesion().format(DateTimeFormatter.ofPattern("MMM yyyy")),
                        Collectors.counting()
                ));
    ...
    public ChartDataDTO generateSkillsRadarChart(Long clubId) {
        ...
        for (Membre m : membres) {
            List<MembreTag> tags = membreTagRepository.findTagsByMembreId(m.getIdMembre());
